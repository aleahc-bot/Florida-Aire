...
// ---- Graceful image fallback: hide broken images so the branded block shows ----
(function(){
  function flag(img){ if(!img.getAttribute('src')) return; img.classList.add('img-failed'); }
  document.querySelectorAll('img').forEach(function(img){
    img.addEventListener('error', function(){ flag(img); });
    if(img.complete && img.naturalWidth === 0) flag(img); // already failed before JS ran
  });
})();
